...
\begin{enumerate}
    ...
    \item Finding Shear Demand, $V^{(1)}_u$ (ignoring self weight):
        \begin{equation*}
            ...
            \end{aligned}
        \end{equation*}
    \item Assuming the deck provides lateral bracing ($L_b = 0$), we can use table 3-2 to find a section with adquate flexural strength ($\Phi _b M_p$) 
        \newline
        Selecting W10X22, 
            \begin{equation*}
                \begin{aligned}
        ...
        \begin{equation*}
            \begin{aligned}
                w^{(2)}_D   &= (\text{Conc SW}) (\text{TW}) + (\text{Beam SW}) \\
                            &= 459lb/ft + 22lb/ft \\
                            &= 481lb/ft \\
                w^{(2)}_L &= (\text{CL}) (\text{TW}) \quad \text{Construction Load}\\
                            &= (20psf)(9ft) \\
